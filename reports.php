			...
			<ul>
				...
				<li class="has-sub active">
					...
					<ul class="sub">
						...
						<li  class="selected active"><a href="reports.php">Reports</a></li>
					</ul>
				</li>
               ...
               <div class="span12">
                  <div class="tabbable tabbable-custom boxless">
                     <ul class="nav nav-tabs">
                        <li class="active"><a href="#opp_won_tab" data-toggle="tab">Opportunity Report</a></li>
                        <li><a class="advance_form_with_chosen_element" href="#task_tab" onClick="report_task()" id="report_t" data-toggle="tab">Task Report</a></li>
                          <li><a class="advance_form_with_chosen_element" href="#campaign_tab" onClick="report_campaign()" id="report_campaign" data-toggle="tab">Campaign Report</a></li>
								...
								<div class="row-fluid">
									<div class="span12">
										...
										<div class="tabbable tabbable-custom">
											...
											<div id="opps_won/lost" style="width: 100%; height: 360px;">
													<center>
													<div class="screen">
													<h2>&nbsp;You haven't selected a specific year and quarter</h2>
														<h3>&nbsp;Please select a year and a quarter</h3></div>
											</center>
													</div> 
												</div>
										...
										<div id="opps_won/lost" style="width: 100%; height: 360px;">
													<center><div class="screen">
													<h2>&nbsp;You haven't selected a specific year and month</h2>
													<h3>&nbsp;Please select a year and a month</h3></div></center>
													</div> 
												</div>
											</div>
										</div>
								...
								<div class="row-fluid">
									<div class="span12">
										...
										<div class="tabbable tabbable-custom">
											...
											<div class="tab-content">
												<div class="tab-pane active" id="quarters_task_tab">
													<p>
								<div class="row-fluid">
									<div class="span12">
										<!--BEGIN TABS-->
	...
	function selectquart(){
		 ...
		 var y = document.getElementById("selyear").value;
			if (x == "First Quarter"){
				var startq = "-01-01";
				var finishq = "-03-31";
				window.location.href ="reports.php?start=" + y + startq + "&finish=" + y + finishq;	
			}
	...
	function selectmonth(){
		 ...
		 var y = document.getElementById("selyear").value;
			if (x == "January"){
				var startq = "-01-01";
				var finishq = "-01-31";
				window.location.href ="reports.php?start=" + y + startq + "&finish=" + y + finishq + "&active=1";	
			}
			else if (x == "February"){
				var startq = "-02-01";
				var finishq = "-02-28";
...
<script>
 var rept= document.getElementById('report_t');
	var repc= document.getElementById('report_campaign');
	rept.addEventListener('click', function(){
						  document.location.href ='reports_task_added.php';
						  });
	repc.addEventListener('click', function(){
		document.location.href = 'reports_campaign.php';
	});
...
function drawAxisTickColors() {
      var data = google.visualization.arrayToDataTable([
		['Accounts', 'Sales/s Won', 'Sales/s Lost'],
       <?php
		$sql1= mysqli_query($conn,"SELECT * FROM tbl_account");
		while($row1 = $sql1->fetch_assoc()){
		$id1= $row1["id"];
		$sql2 = mysqli_query($conn,"SELECT * FROM tbl_opportunities WHERE (account_idr ='" .$id1."') AND (close_date_dat BETWEEN '".$start."' AND '".$finish."')");
		$total=0;
		$total1=0;
		while($row2=$sql2->fetch_assoc()){
			...
			echo "['" .$row1["account_name_str"]. "'," .$total. "," .$total1. "],";
		}
				?>
      ]);
      ...
      var options = {
        title: 'Opportunities Won/Lost <?php echo " from " . $start ." to ". $finish;?>',
		 colors: ['green', 'red'],
      ...
      var chart = new google.visualization.BarChart(document.getElementById('opps_won/lost'));
...
<script type="text/javascript">
    google.charts.load('current', {'packages':['bar']});
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Accounts', 'Task/s Added', 'Task/s Completed'],
          <?php 
				$sql = mysqli_query($conn, "SELECT account_name_str, address_lng, city_str, country_idr, postcode_int, phone_str, website_str, notes_lng, account_type_idr, owner_idr, task_ctr, task_completed_ctr FROM tbl_account");

				while($row = $sql->fetch_assoc()){
					echo "['" .$row["account_name_str"]. "'," .$row["task_ctr"]. "," .$row["task_completed_ctr"]. "],";
				}
        ...
        var options = {
          ...
          height: 350,
		  width: 800,
          colors: ['blue', 'green', '#7570b3']
        };

        var chart = new google.charts.Bar(document.getElementById('tasks_added/completed'));

        chart.draw(data, google.charts.Bar.convertOptions(options));

        var btns = document.getElementById('btn-group_task');
